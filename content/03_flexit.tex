...
\subsection{CLIP embedding module} 
...
classification networks (more details in appendix). \todo{add appentix}
...
in the first three columns of Figure~\ref{fig:failures}. Secondly, our method is limited to the capacities 
of the multimodal embedding space, which is limited to 

show representative failure cases for our method, due to either the regularization 
 method or the multimodal embedding space.
...
Moreoever, while we studied transformations that change the class or color of the main 
...
As our algorithm relies on \ac{CLIP} for editing, it could potentially inherit its biases. 
The authors of \ac{CLIP} have demonstrated that their model is subject to fairness issues 
...
prompted transformations such as doctor $\rightarrow$ newscaster, although we have not
 ...
 to add constraints with \ac{CLIP} prompts to control bias before and after editing.
